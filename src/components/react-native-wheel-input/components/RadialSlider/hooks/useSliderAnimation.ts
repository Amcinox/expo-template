...
const useSliderAnimation = (props: RadialSliderAnimationHookProps) => {
  ...
  useEffect(() => {
    // Don't update internal state if the change was triggered by this component
    if (!isInternalChange.current && min <= props?.value && max >= props?.value && props?.value !== value) {
  ...
  const handlePanResponderMove = (
    ...
    // Set the flag to indicate internal change
  ...
  const handlePanResponderEnd = () => {
    ...
    // Reset the flag after internal change is complete
    setTimeout(() => {
