...
import { normalizeTheme } from '@/utils/colors';
...
type SettingsAction =
    ...
    | { type: SettingsActionType.UPDATE_LOGOS; payload: { textLogo: string | null; iconLogo: string | null } };
...
const handlers: Record<string, (state: SettingsState, action: SettingsAction) => SettingsState> = {
    ...
    UPDATE_LOGOS: (state, action) => {
        return {
            ...state,
            logos: {
                textLogo: action.payload.textLogo,
                iconLogo: action.payload.iconLogo
            }
        }
    },
...
export function SettingsProvider({ children }: SettingsProviderProps) {
    ...
    const registerForPushNotificationsAsync = useCallback(async () => {
        ...
        console.log("Getting push token for push");
        ...
        if (finalStatus !== 'granted') {
            ...
            throw new Error('Failed to get push token for push');
            return;
        }
        ...
        try {
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new Error('Push notification token request timed out after 4 seconds')), 4000);
            });

            const pushTokenString = await Promise.race([
    ...
    const getBiometricInfo = useCallback(async () => {
        try {
            ...
            const mappedAuthenticators = availableAuthenticators.map(authType => {
                switch (authType) {
            ...
            return {
                ...
                biometricType: mappedAuthenticators
    ...
    useEffect(() => {
        const loadAsyncStorage = async () => {
            try {
                ...
                const [
                    ...
                    getMultiFromAsyncStorage(
                        [
                            ...
                            { key: InitialState.logos, defaultValue: initialState.logos },
                        ]),
                    getDeviceInfo(),
                ...
                const { colorSchemeName, language, walkThrough, theme, logos } = storageSettings;
                ...
                const initPayload: Partial<SettingsState> = {
                    ...
                    walkThrough,
                    logos,
                    theme: normalizeTheme(theme as Theme, initialState.theme),
                    device: {
                        ...device,
                        pushNotificationToken
                    },
    ...
    const updateTheme = useCallback(async (theme: Theme) => {
        // save to async storage
        const normalizedTheme = normalizeTheme(theme, initialState.theme);
        await saveItem(InitialState.theme, normalizedTheme)
    ...
    const updateLogos = useCallback(async ({ textLogo, iconLogo }: {
        textLogo: string | null;
        iconLogo: string | null;
    }) => {
        // save to async storage
        await saveItem(InitialState.logos, { textLogo, iconLogo })
        // update state
        dispatch({
            type: SettingsActionType.UPDATE_LOGOS,
            payload: {
                textLogo,
                iconLogo
            }
