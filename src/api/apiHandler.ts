...
import { authorizationHandler } from '@/utils/auth-helpers';
  ...
  constructor(baseURL: string, config?: AxiosRequestConfig) {
    ...
    this.axiosInstance.interceptors.request.use(
      async (config) => {
        ...
        const storedAccessToken = await SecureStore.getItemAsync(StorageKeys.ID_TOKEN_KEY);
        if (storedAccessToken) {
          config.headers.Authorization = authorizationHandler(storedAccessToken)
        } else {
      ...
      async (error) => {
        ...
        if (error.response) {
          ...
          return Promise.reject({
            message: errorHandler(error, 'ERR_API_ERROR'),
